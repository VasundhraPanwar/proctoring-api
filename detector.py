...
def transcribe_audio(audio_path: str) -> Optional[dict]:
    ...
    Handles long files by splitting into 10-minute chunks (Whisper limit = 25MB).
    Reassembles full transcript with corrected timestamps.
    """
    try:
...
def split_audio(audio_path: str, chunk_s: int) -> list:
    """
    Split audio into chunk_s second segments.
    Returns list of (chunk_path, offset_s) tuples.
    """
    ...
    while offset < duration:
        ...
        if result.returncode == 0 and Path(chunk_path).exists():
            chunk_mb = Path(chunk_path).stat().st_size / 1024 / 1024
            if chunk_mb > 0.1:  # skip empty chunks
...
def find_interviewee_region_by_ocr(video_path: str, interviewee_name: Optional[str], frame_w: int, frame_h: int) -> dict:
    """
    3-stage pipeline to find interviewee face region:
    
    Stage 1: OCR — scan frames for name label, get tile above it
    Stage 2: Claude Vision — send a frame, ask "which person is the interviewee?"
    Stage 3: Largest face fallback (least reliable)
    """

    # ── Stage 1: OCR name label ────────────────────────────────────────────────
    if interviewee_name:
        region = _ocr_find_name(video_path, interviewee_name, frame_w, frame_h)
        if region:
            return region
        print("[REGION] OCR failed — trying Claude Vision")

    # ── Stage 2: Claude Vision ─────────────────────────────────────────────────
    region = _claude_vision_find_interviewee(video_path, interviewee_name, frame_w, frame_h)
    if region:
        return region
    print("[REGION] Claude Vision failed — using largest face fallback")

    # ── Stage 3: Largest face fallback ────────────────────────────────────────
...
def _ocr_find_name(video_path: str, interviewee_name: str, frame_w: int, frame_h: int) -> Optional[dict]:
    """
    Scan multiple frames with pytesseract.
    Look for interviewee name label (white text at bottom of video tile).
    Returns tile bounding box or None.
    """
    try:
        import pytesseract
        ...
        # Scan frames spread across first 3 minutes
        for t in [5, 15, 30, 60, 90, 120, 180]:
            ...
            # Meet/Zoom name labels appear in bottom 35% of frame
            strip_y    = int(frame_h * 0.65)
            bottom     = frame[strip_y:, :]
            gray       = cv2.cvtColor(bottom, cv2.COLOR_BGR2GRAY)

            # Try multiple thresholds to catch different UI styles
            for thresh_val in [160, 180, 200]:
                ...
                for i, word in enumerate(data["text"]):
                    ...
                    if first_name in word_lower or (last_name and last_name in word_lower):
                        ...
                        # Tile = region above the name label
                        # Meet tiles are roughly 16:9, name at very bottom
                        tile_x1 = max(0,      nx - 60)
...
def _claude_vision_find_interviewee(video_path: str, interviewee_name: Optional[str], frame_w: int, frame_h: int) -> Optional[dict]:
    """
    Send a frame to Claude Vision.
    Ask: which person is the interviewee and where are they on screen?
    Claude reads name labels, tile positions, and layout directly.
    ...
    try:
        ...
        # Try a few frames to get a good one (avoid black frames)
        frame_to_use = None
        for t in [30, 60, 15, 90, 10]:
            cap.set(cv2.CAP_PROP_POS_MSEC, t * 1000)
            ret, frame = cap.read()
            if ret and frame is not None:
                # Check frame is not mostly black
        ...
        # Encode frame as JPEG for Claude
...
Tell me the INTERVIEWEE's position on screen by giving me approximate pixel coordinates of their video tile.
...
{{
  ...
  "tile_x1": left edge pixel,
  "tile_y1": top edge pixel,
  "tile_x2": right edge pixel,
  "tile_y2": bottom edge pixel,
  "confidence": "HIGH or MEDIUM or LOW",
  "reasoning": "brief explanation of how you identified them"
        ...
        print(f"[VISION] Claude identified: '{parsed.get('interviewee_name_on_screen')}' at ({x1},{y1})→({x2},{y2}) confidence={parsed.get('confidence')}")
...
def classify_gaze_direction(yaw: float, pitch: float, gaze_x: float) -> str:
    ...
    Uses head pose (yaw/pitch) as primary signal.
    Iris gaze is unreliable alone — only use as secondary confirmation.
    CENTER zone is intentionally wide to avoid false positives.
    Normal thinking glances (looking up/sideways briefly) = CENTER.
    """
    # Head pose — only flag clear deliberate turns
    if yaw < -30:
        return "LEFT"
    if yaw > 30:
        return "RIGHT"
    if pitch < -20:   # clearly looking down at desk/lap
        return "DOWN"
    if pitch > 25:    # clearly looking up (thinking = normal, but track it)
        return "UP"

    # Within normal head range — use iris only if strongly off-center
    if gaze_x < 0.32:
        return "LEFT"
    if gaze_x > 0.68:
        return "RIGHT"

    # Everything else = CENTER (includes normal thinking glances)
    return "CENTER"
...
def detect_direction_patterns(gaze_log: list, min_occurrences: int = 3, window_s: float = 60.0) -> list:
    ...
    for direction in ["LEFT", "RIGHT", "DOWN"]:  # UP excluded — normal thinking behavior
...
class InterviewCheatingDetector:
    def __init__(
        ...
        dir_min_occurrences:    int   = 5,   # need 5+ looks in same direction
        dir_window_s:           float = 90.0, # within a 90s window
    ):
    ...
    def analyze(self) -> DetectionResult:
        ...
        mp_face_detect.FaceDetection(
            ...
            while True:
                ...
                # NOTE: Do NOT flag absent — interviewer camera off causes tile layout
                # shifts which temporarily loses the interviewee face.
                if n_faces == 0:
                    gaze_log.append({"time_s": round(current_time,2), "direction": "ABSENT"})
                    continue  # not a violation — could be camera/lighting issue

                # Only flag 3rd person if BOTH faces are large (> 8% of crop area)
                # This filters out:
                #   - Interviewer thumbnail in corner of screen
                #   - Small background faces
                #   - UI artifacts
                # A real coaching person would be nearly as large as the interviewee
                real_faces = sum(
                ...
                if mesh.multi_face_landmarks:
                    ...
                    # Only flag LEFT/RIGHT/DOWN — UP = thinking, not cheating
        ...
        # Boost only if gaze is HEAVILY skewed to one direction (> 50% of time)
        # 30% was too sensitive — normal interviews have lots of off-center looks
