...
async def run_job(job_id: str, video_path: Path, student_id: str):
    ...
    print(f"[JOB START] job_id={job_id} student={student_id} file={video_path}")
    try:
        ...
        print(f"[JOB DONE] job_id={job_id} score={result.get('cheating_score')} risk={result.get('risk_level')}")
        update_job(job_id,
            status       = "done",
        ...
        import traceback
        print(f"[JOB FAILED] job_id={job_id} error={traceback.format_exc()}")
        update_job(job_id,
            status       = "failed",
            completed_at = datetime.utcnow().isoformat(),
            error        = str(e),
