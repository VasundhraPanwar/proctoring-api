    ...
    def analyze(self) -> DetectionResult:
        ...
        mp_face_detect.FaceDetection(
            ...
            while True:
                ...
                # Only flag 3rd person if face is large enough (not an avatar/thumbnail)
                # and genuinely inside the interviewee's crop region
                real_faces = sum(
                    1 for det in (fd.detections or [])
                    if det.location_data.relative_bounding_box.width *
                       det.location_data.relative_bounding_box.height > 0.02
                )
                if real_faces > 1:
