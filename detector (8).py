...
{{
  ...
  "interviewer_side": "LEFT or RIGHT or TOP or BOTTOM or NONE — which side of the interviewee is the interviewer on?",
  "confidence": "HIGH or MEDIUM or LOW",
...
def classify_gaze_direction(yaw: float, pitch: float, gaze_x: float, interviewer_side: str = "NONE") -> str:
    """
    Returns: LEFT, RIGHT, DOWN, CENTER

    interviewer_side: "LEFT", "RIGHT", or "NONE"
    Glances TOWARD the interviewer = normal eye contact = CENTER (not flagged).
    Only flag glances AWAY from interviewer = toward hidden notes/screen.
    UP is never flagged — looking up = thinking = completely normal.
    """
    # Determine raw direction from head pose (primary) + iris (secondary)
    if yaw < -28:
        raw = "LEFT"
    elif yaw > 28:
        raw = "RIGHT"
    ...
    # Looking TOWARD the interviewer = normal eye contact = not suspicious
